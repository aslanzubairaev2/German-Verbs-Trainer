...
const GermanVerbsApp = () => {
    ...
    const fetchGeminiInfo = useCallback(async (verb, setter, force = false) => {
      ...
      const apiKey = process.env.REACT_APP_GEMINI_API_KEY; // Убедитесь, что ключ доступен
      if (!apiKey) {
          setter({ loading: false, data: null, error: "API ключ не настроен." });
          return;
      }
      
      // --- НОВЫЙ, БОЛЕЕ ПОДРОБНЫЙ ПРОМПТ ---
      const prompt = `
        ...
        Пример для "ich mache":
        {
          ...
          "forms": {
            "present": {
              "question": "Mache ich meine Hausaufgaben?",
              "affirmative": "Ich mache meine Hausaufgaben.",
              "negative": "Ich mache meine Hausaufgaben nicht."
            },
            "past": {
              "question": "Habe ich meine Hausaufgaben gemacht?",
              "affirmative": "Ich habe meine Hausaufgaben gemacht.",
              "negative": "Ich habe meine Hausaufgaben nicht gemacht."
            },
            "future": {
              "question": "Werde ich meine Hausaufgaben machen?",
              "affirmative": "Ich werde meine Hausaufgaben machen.",
              "negative": "Ich werde meine Hausaufgaben nicht machen."
            }
          }
        }
      ...
      try {
          ...
          if (Array.isArray(parsedJson)) parsedJson = parsedJson[0]; // Handle cases where it returns an array
